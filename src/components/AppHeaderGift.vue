<template>
    <div class="bg-nav border-b border-gray-200 dark:border-gray-700">
        <ul class="grid grid-cols-2 -mb-px text-sm font-medium text-center text-gray-500 dark:text-gray-400">
            <li class="mr-2">
                <a
                    @click="changeShowContentsType('study')" :class="{'active': show == 'study'}"
            ...
            <li class="mr-2">
                <a
                    @click="changeShowContentsType('refresh')" :class="{'active': show == 'refresh'}"
                    class="inline-flex p-2 border-b-2 border-transparent rounded-t-lg hover:text-gray-600 hover:border-gray-300 dark:hover:text-gray-300 group"
                    aria-current="page">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="mr-2 text-gray-400 w-5 h-5">
...
export default {
  ...
  methods: {
    ...
    setShowContentsGiftType : function(){
      this.$store.commit('setShowContentsGiftType', this.show);
    },
  },
}
</script>
